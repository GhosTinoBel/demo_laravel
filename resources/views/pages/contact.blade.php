...
<div class="col-md-6 col-md-offset-1">
     <form action="{{ route('contact_path') }}" method="POST" novalidate>
               ...
               <div class="form-group {{ $errors -> has('prenom') ? 'has-error' : ''}}">
                    ...
                    <input type="text" class="form-control" name="prenom" id="prenom" placeholder="vos prénoms -si vous en avez plusieurs" value="{{ old('prenom') }}"  required="required">
                    {!! $errors -> first('prenom', '<span class="help-block">:message</span>') !!}
               </div>
               <div class="form-group {{ $errors -> has('email') ? 'has-error' : ''}}">
                    <label for="email" class="control-label">Email</label>
                    <input type="email" class="form-control" name="email" id="email" placeholder="votre adresse mail" value="{{ old('email') }}" required="required">
                    {!! $errors -> first('email', '<span class="help-block">:message</span>') !!}
               </div>
               <div class="form-group {{ $errors -> has('message') ? 'has-error' : ''}}">
                    <label for="message" class="control-label">Votre message</label>
                    <textarea class="form-control" rows="3" name="message" id="message"  placeholder="Alors, Que voudriez-vous me dire ?" value="{{ old('message') }}" required="required"></textarea>
